...
public class AccessLoggingHttpMessageReceiverTest extends GreenbirdMuleFunctionalTestCase {
    @Override
    ...
    public void connect_multipleCalls_transformersOnlyAddedOnce() throws Exception {
        InboundEndpoint endpoint = bean("http-test-endpoint");
        new TestReceiver(bean(HttpsConnector.class), flow("https-test-flow"), endpoint).test();

    }

    private static class TestReceiver extends AccessLoggingHttpMessageReceiver {
